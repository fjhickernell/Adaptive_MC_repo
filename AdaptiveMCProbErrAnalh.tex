...
\begin{theorem} \label{costtheorem} The two stage Monte Carlo algorithm for fixed width confidence intervals based on i.i.d.\ sampling described in Algorithm \ref{twostagealgo}, denoted $\aMC$, has a probabilistic cost bounded above by 
\begin{equation*}
...
Note that the Chebychev sample size, $N_{\text{Cheb}}$, defined in \eqref{NCdef}, the Berry-Esseen sample size, $N_{\text{BE}}$, and thus $N_\mu$ depend on $\sigma$ and $\varepsilon$ through their ratio, $\sigma/\varepsilon$
