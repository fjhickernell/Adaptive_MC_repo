...
\begin{proposition}\label{propCant} 
...
\begin{gather*}
\Prob\left[\abs{Z-E(Z)} \ge a \right] \le \frac{\var(Z)}{a^2} \qquad \text{Chebychev's inequality \citep[6.1c]{LinBai10a}} , \\
...
The theorem below combines the results on estimating the variance with the sample sizes arising from Chebychev's inequality and the Berry-Esseen inequality.  These lead to an adaptive Monte Carlo algorithm with a probabilistic error guarantee.
