...
Illinois Institute of Technology, Chicago, IL, 60521, USA,
...
\abstract{Monte Carlo methods are used to approximate the means, $\mu$, of random variables $Y$, whose distributions are not known explicitly.  The key idea is that the average of a random sample, $Y_1, \ldots, Y_n$, tends to $\mu$ as $n$ tends to infinity. This article explores how one can confidently construct a confidence interval for $\mu$ with a prescribed half-width (or error tolerance) $\varepsilon$.  Our proposed two stage algorithm assumes that $Y$ has bounded \emph{kurtosis}. An initial independent and identically distributed (IID) sample is used to confidently estimate the variance of $Y$.  A Berry-Esseen inequality then makes it possible to determine the size of the IID sample required to construct the desired confidence interval for $\mu$.  We discuss the important case where $Y=f(\vX)$ and $\vX$ is a random $d$-vector with probability density $\rho$.  In this case $\mu$ can be interpreted as the integral $\int_{\reals^d} f(\vx) \rho(\vx) \, \dif \vx$, and the Monte Carlo method becomes a method for multidimensional cubature. }
...
width confidence intervals of the form \eqref{confint}. Moreover, we want to make sure that our confidence interval is correct, not approximately correct, or correct in the limit of vanishing $\varepsilon$.  In this paper we present Algorithm \ref{twostagealgo} for obtaining such a fixed width confidence interval for the mean of a real random
...
\mu=\e(Y) = \int_{\reals^d} f(\vx) \rho(\vx) \, \dif \vx.
\end{equation}
...
\begin{align} 
\nonumber
\Prob\left[\abs{\mu-\hmu_n}  \le \frac{\sigma}{\sqrt{n}}x\right]&=\Prob\left[\hmu_n - \mu \le \frac{\sigma}{\sqrt{n}}x\right]-\Prob\left[\hmu_n - \mu < -\frac{\sigma}{\sqrt{n}}x\right]\\ 
\nonumber
...
Note that if the term in \eqref{NB} with the numerator $0.56 M$ were absent, $N_{\text{BE}}$ would correspond to the CLT sample size $N_{\text{CLT}}$, and in general $N_{\text{BE}}>N_{\text{CLT}}$.   It is possible that in some situations
...
The ideal case of optimizing $n_\sigma$ and $\fudge$ with respect to $\sigma/\varepsilon$ is impractical, since $\sigma$ is not known in advance.  Our suggestion is to choose $\fudge$ around $1.2$--$1.5$, and then choose $n_\sigma$ as large as needed to ensure that $\tkappa_{\max}$ is as large as desired. For example with  $\fudge = 1.5$ and 
$\tilde \kappa = 10, 100, 1000$
we get $n_\sigma = 2888$, $31755,$ and $320440$ respectively.
...
These $500$ random constructions of $f$ with $d=1$ are integrated using {\tt quad},  {\tt quadgk}, {\tt chebfun}, Algorithm \ref{twostagealgo}, and an automatic quasi-Monte Carlo algorithm that uses scrambled Sobol' sampling \citep{Owe95,Owe96,Owe97,Mat98,HonHic00a,DicPil10a} with quasi-standard error \citep{Hal05a,Owe06a} to determine the sample size.  We do not yet have a theorem providing sufficient conditions under which this quasi-Monte Carlo algorithm is guaranteed to provide the answer within the specified error tolerance, and so we do not describe this automatic Sobol' sampling algorithm in detail.  
...
Letting $Y^{(d)}$ denote the discretization of \eqref{payoff} using $d$ trapezoids, we approximated $\mu^{(d)}=\e(Y^{(d)})$ by Algorithm \ref{twostagealgo} and the Sobol' sampling algorithm using an error tolerance of $\varepsilon=0.02$, and compared it to the analytic value of $\mu^{(d)}$.  The result of $500$ replications is given in Figure \ref{AsianGeoMeanCallFig}.  Some of the parameters were set to be fixed values, namely,
\[
S(0)=K=100, \qquad T=1, \qquad r=0.03.
\]
The volatility, $v$, was drawn uniformly between $0.1$ and $0.8$.  The number of trapezoids, $d$, was chosen to be uniform over $\{2, 4, 8, 16, 32\}$.  The true value of $\mu^{(d)}$ for these parameters is between about $4$ and $20$.
...
\caption{Execution times and errors for the Asian geometric mean call option for $d=2, 4, 8, 16, 32$ and $\varepsilon=0.02$.\label{AsianGeoMeanCallFig}}
\end{figure}

For this example the true kurtosis of $Y^{(d)}$ is unknown.  Both  Algorithm \ref{twostagealgo} and the  Sobol' sampling algorithm compute the option price with high reliability.  ?????  

\section{Discussion}

Practitioners often construct CLT-based confidence intervals with the true variance estimated by the sample variance multiplied by some inflation factor.  Often, this approach works, but it has no guarantee of success.  The two stage algorithm presented here is similar to the approach just described, but it carries guarantees.  These are derived by employing Cantelli's inequality to ensure a reliable variance upper bound, and by employing a Berry-Esseen inequality to ensure a large enough sample for the sample mean.  

In certain cases our procedure multiplies the computational cost by a large factor such as $4$ or $10$ or even $100$ compared to what one might spend based on the CLT with a known value of $\sigma$.  While this seems inefficient, it is well to remember that the total elapsed time may still be well below several seconds.  It is also well to remember that one typically does not know $\sigma$ in advance, and our adaptive algorithm estimates $\sigma$ and then an appropriate sample size $n_\mu$ from the data.  Our algorithmic cost will be low when the unknown $\sigma$ is small and large when $\sigma$ is large.

Like any algorithm with guarantees, our algorithm does need to make assumptions about the random variable $Y$.  We assume a bound on the kurtosis of $Y$.  In practice we may not know a priori if our random variable satisfies this bound.  However, if our algorithm fails to construct a confidence interval with the prescribed width, the guarantee in Theorem \ref{mainadaptthm} points to the culprit of a large kurtosis.  The user may then adjust the algorithm by increasing $n_\sigma$ and/or $\fudge$.
...
Our algorithm is adaptive because $n_\mu$ is determined from the sample variance.  Information-based complexity theory tells us that adaptive information does not help for the integration problem for convex sets of integrands in the worst case and probabilistic settings \citep[Chapter 4, Theorem 5.2.1; Chapter 8, Corollary 5.3.1]{TraWasWoz88}.  Here, in Corollary \ref{integcor} the cone, $\cc_{\kappa_{\max}}$, is not a convex set, so adaption can help.
